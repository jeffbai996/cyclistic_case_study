...
We will also make a backup of the merged data frame, as we want to keep a copy of the dataset pre-cleaning.
...
df_bike_test$day_of_week <- wday(df_bike_test$started_at)
```
...
Then, we remove columns that are not relevant to the analysis, namely trip ID, ...